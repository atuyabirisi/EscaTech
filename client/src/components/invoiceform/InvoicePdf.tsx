import { useParams } from "react-router-dom";
import { InvoiceData } from "../../types/typeDefinitions";
import useDataObject from "../../hooks/useDataObject";

export default function InvoicePdf() {
  const { id } = useParams();
  const { data } = useDataObject<InvoiceData>(`/invoice/${id}`);

  return (
    <>
      <div className="card">
        <div className="card-header pdfcolor" />
        <div className="card-body">
          <div className="d-flex justify-content-end">
            <div>
              <h4>
                <b>Escatech Services Ltd</b>
              </h4>
              <h6>P.O.BOX 58851-00200, Nairobi</h6>
              <h6>Tel: 0720 532586</h6>
              <h6>Email: escatech2012@gmail.com</h6>
              {/* <h6>PIN: PO51346375, VAT: 03268668H</h6> */}
            </div>
          </div>
          <div className="row mb-4">
            <div className="col-3">
              <h3>
                <b>Invoice</b>
              </h3>
              <h6>Invoice no: {data?._id}</h6>

              <h6>Due Date: {Date.now()}</h6>
            </div>
            <div className="col-9 border-bottom" />
          </div>
          <div className="d-flex justify-content-between mb-5">
            <div>
              <h5>
                <b>Issued to:</b>
              </h5>
              <h6>{data?.client.name}</h6>
              <h6>{data?.client.address}</h6>
              <h6>{data?.client.email}</h6>
            </div>

            <div>
              <h5>
                <b>Payment Information:</b>
              </h5>
              <h6>Bank: Kenya Commercial Bank</h6>
              <h6>Account Name: Escatech Services Ltd</h6>
              <h6>Account Number: 425242323487</h6>
            </div>
          </div>
          <div className="mb-4">
            <table
              className="table table-bordered table-striped rounded my-4"
              style={{ overflow: "hidden" }}
            >
              <thead className="table-light text-center">
                <tr>
                  <th scope="col">Description</th>
                  <th scope="col">Qty</th>
                  <th scope="col">Price</th>
                  <th scope="col">Total</th>
                </tr>
              </thead>
              <tbody className="text-center">
                {data?.products.map((product, index) => (
                  <tr key={index}>
                    <td>{product.description}</td>
                    <td>{product.quantity}</td>
                    <td>{product.unitprice}</td>
                    <td>{product.amount}</td>
                  </tr>
                ))}
                <tr className="border">
                  <td rowSpan={3} />
                  <td colSpan={2}>
                    <b>SUBTOTAL</b>
                  </td>
                  <td>
                    <b>{data?.total}</b>
                  </td>
                </tr>
                <tr>
                  <td colSpan={2}>
                    <b>V.A.T</b>
                  </td>
                  <td>
                    <b>{data?.vat}</b>
                  </td>
                </tr>
                <tr>
                  <td colSpan={2}>
                    <b>GRAND TOTAL</b>
                  </td>
                  <td>
                    <b>{data?.grandTotal}</b>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
          <div className="row mt-4">
            <div className="col-lg-4 d-flex flex-column justify-content-center text-center">
              <div className="border-bottom mb-1">
                <h5>Terms and Conditions</h5>
              </div>
              <p>
                Please make payment within 30 days of receiving this
                invoice.There will be a 10% interest charge per month on late
                payment
              </p>
            </div>

            <div className="col-lg-4" />
            <div className="col-lg-4 text-center">
              <div className="border-bottom">
                <img src="\src\assets\sig.PNG" alt="signature" />
              </div>
              <div className="border-bottom" />
              <h5>Alex Gakanga</h5>
              <h6>Director</h6>
            </div>
          </div>
        </div>
        <div className="card-footer border-2 text-center  pdfcolor">
          <small>Escatech services ltd</small>
          <br />
          <small>
            Dealers in installation, repair and maintenance of lifts and
            escalators, electrical services
          </small>{" "}
          <br />
          <small>Invoice Generated on: {Date.now()}</small>
        </div>
      </div>
    </>
  );
}
